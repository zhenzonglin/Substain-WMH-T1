...
from .wmh_window import WMH_MODEL_MAX_SHAPE, WMH_MODEL_STRIDE
...
def stage_lesion(config: Mapping[str, object], participant: Participant) -> Dict[str, object]:
    ...
    lesion_mni152 = output / "lesion_space-FSLMNI152.nii.gz"
    ...
    for path in fsl_references:
        if same_grid(participant.lesion_mask, path):
            matches.append((path, "exact"))
        elif same_world_lattice(participant.lesion_mask, path):
            matches.append((path, "world_lattice_equivalent"))
    if len(matches) != 1:
        raise ValueError("病灶输入不匹配FSL MNI152标准1mm或2mm网格")
    source_reference, grid_relation = matches[0]

    # 先统一成项目固定FSL网格，兼容同一物理格点的RAS/LAS或轴置换存储。
    resample_label_to_reference(participant.lesion_mask, source_reference, lesion_mni152)
    _force_binary(lesion_mni152)
    if not same_grid(lesion_mni152, source_reference):
        raise RuntimeError("病灶未能标准化到固定FSL MNI152参考网格")
    source_voxels = int(np.count_nonzero(source_data))
    standardized_voxels = int(np.count_nonzero(np.asanyarray(nib.load(str(lesion_mni152)).dataobj)))
    if source_voxels != standardized_voxels:
        raise RuntimeError(
            "FSL MNI152等价格点标准化后病灶体素数改变: {} -> {}".format(
                source_voxels, standardized_voxels
            )
        )
    template = _absolute(root, config["wmh"]["template"])  # type: ignore[index]
    resample_label_to_reference(lesion_mni152, template, lesion_ch2better)
    ...
    return {
        ...
        "lesion_mni152_standardized": str(lesion_mni152),
        ...
        "source_grid_relation": grid_relation,
        "source_grid_normalized": True,
        "source_voxel_count": source_voxels,
        "standardized_voxel_count": standardized_voxels,
        "mni152_to_ch2better": "world_coordinate_resampling_after_fsl_grid_normalization",
...
def stage_skullstrip(config: Mapping[str, object], participant: Participant, profile: str) -> Dict[str, object]:
    """仅执行需要GPU排队的T1/FLAIR SynthStrip，不在锁内运行ANTs。"""
...
def stage_registration(config: Mapping[str, object], participant: Participant) -> Dict[str, object]:
    """消费已完成的SynthStrip结果，仅在CPU资源池中运行ANTs配准。"""
...
def stage_wmh_segmentation(config: Mapping[str, object], participant: Participant, profile: str) -> Dict[str, object]:
    """使用SynthStrip脑掩膜选择自适应窗口，再生成原生FLAIR WMH结果。"""

    root = Path(str(config["project_root"]))
    output = participant_dir(config, participant) / "wmh"
    # 模型网格仅用于推理；最终硬分割和概率图必须按世界坐标返回原生FLAIR。
    ...
    window_json = output / "WMHSynthSeg_window.json"
    skullstrip_status = _read_json(status_path(config, participant, "skullstrip"))
    if skullstrip_status.get("status") != "pass":
        raise RuntimeError("依赖节点skullstrip失败，禁止运行WMH-SynthSeg")
    flair_brain_mask = Path(str(skullstrip_status.get("details", {}).get("flair_brain_mask", "")))
    if not flair_brain_mask.is_file():
        raise FileNotFoundError("缺少前序FLAIR SynthStrip脑掩膜 {}".format(flair_brain_mask))
    device = str(config["execution"]["device_gpu" if profile == "gpu" else "device_cpu"])  # type: ignore[index]
    ...
    expected_provenance = {
        "schema_version": "1.0",
        "flair_sha256": sha256(participant.flair),
        "flair_brain_mask_sha256": sha256(flair_brain_mask),
        "model_sha256": sha256(model),
        "runtime_sha256": sha256(runtime),
        "device": device,
        "adaptive_window_method": "flair_synthstrip_mask_v1",
        "model_window_max_shape": list(WMH_MODEL_MAX_SHAPE),
        "model_padding_multiple": WMH_MODEL_STRIDE,
        "crop_enabled": True,
    ...
    reusable = (
        all(path.is_file() for path in (model_grid_seg, model_grid_probability, volumes_csv, window_json))
    ...
    window = _read_json(window_json)
    if window.get("method") != "flair_synthstrip_mask_v1" or float(window.get("brain_mask_coverage", 0.0)) != 1.0:
        raise RuntimeError("WMH-SynthSeg自适应窗口记录无效")
    ...
    if not same_grid(seg, participant.flair) or not same_grid(probability, participant.flair):
        raise RuntimeError("WMH-SynthSeg结果未恢复到原生FLAIR网格")
    return {
        ...
        "window_metadata": str(window_json),
        "device": device,
        "inference_reused": reusable,
        "adaptive_window_method": window["method"],
        "model_input_shape_1mm": window["input_shape_1mm"],
        "model_window_shape_1mm": window["window_shape_1mm"],
        "model_tensor_shape": window["tensor_shape"],
        "model_crop_start": window["crop_start"],
        "model_crop_stop": window["crop_stop"],
        "brain_bbox_shape_1mm": window["brain_bbox_shape"],
        "brain_mask_coverage": window["brain_mask_coverage"],
        "model_window_max_shape": window["max_window_shape"],
        "model_padding_multiple": window["padding_multiple"],
        "cropped_background_only": window["cropped_background_only"],
        "runtime_sha256": expected_provenance["runtime_sha256"],
        "flair_brain_mask_sha256": expected_provenance["flair_brain_mask_sha256"],
...
def aggregate_outputs(config: Mapping[str, object], participants: Sequence[Participant]) -> Dict[str, object]:
    ...
    for participant in participants:
        ...
        # 失败病例的影像产出已由cleanup删除，因此不得再读取较早阶段残留的pass状态。
...
def _cleanup_failed_outputs(
    ...
    """失败病例只保留status和logs，删除全部影像产出及已生成QC图。"""
    ...
    if central_qc.is_dir():
        ...
        for candidate in sorted(central_qc.glob("{}_*.png".format(participant.participant_id))):
            if candidate.parent.resolve() != central_qc_resolved or candidate.is_symlink() or not candidate.is_file():
                raise RuntimeError("拒绝清理非中央QC普通文件: {}".format(candidate))
            candidate.unlink()
            deleted_qc.append(str(candidate.resolve()))
    ...
    return {
        ...
        "retained_directories": [
            str(subject_root / "status"),
            str(subject_root / "logs"),
        ],
...
def stage_cleanup(config: Mapping[str, object], participant: Participant) -> Dict[str, object]:
    ...
    for key in ("lesion_mni152_standardized", "lesion_ch2better"):
        if lesion.get(key):
